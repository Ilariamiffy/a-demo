<template>
    <el-main class="main-content">
        ...
        <el-form :inline="true" :model="formData" class="form" size="medium">
            ...
            <el-form-item prop="name">
                ...
                <el-autocomplete class="inline-input" v-model="formData.name" :fetch-suggestions="querySearch"
                    placeholder="会员姓名" @select="handleSelect">
            ...
            <el-form-item prop="birthyear">
                <el-date-picker v-model="formData.birthyear" type="month" placeholder="出生年月" value-format="yyyy-MM"
                    clearable>
                </el-date-picker>
            </el-form-item>
            <el-form-item>
        ...
        <el-dialog title="增加会员" :visible.sync="dialogAddVisible" center width="400px">
            <el-form :model="addForm" ref="addForm" :rules="formRules">
                ...
                <el-form-item label="出生年月" prop="birthyear" :label-width="labelWidth">
                    <el-date-picker v-model="addForm.birthyear" type="month" format="yyyy年MM月" value-format="yyyy年MM月"
                        placeholder="出生年月" clearable>
                    </el-date-picker>
                </el-form-item>
        ...
        <el-table :data="transArr.slice((currentPage - 1) * pageSize, currentPage * pageSize)" style="width: 100%">
            ...
            <el-table-column fixed prop="num" label="会员卡号" width="150">
            </el-table-column>
            <el-table-column prop="name" label="姓名" width="120"> </el-table-column>
            <el-table-column prop="birthyear" label="出生年月" width="120">
            </el-table-column>
            <el-table-column prop="tel" label="联系电话" width="120">
            </el-table-column>
            <el-table-column prop="marks" label="可用积分" width="120">
            </el-table-column>
            <el-table-column prop="money" label="可用金额" width="120">
            </el-table-column>
        ...
        <el-dialog title="编辑会员" :visible.sync="dialogEditVisible" center width="400px">
            <el-form :model="editForm" ref="editForm" :rules="formRules">
                ...
                <el-form-item label="出生年月" prop="birthyear" :label-width="labelWidth">
                    <el-date-picker v-model="editForm.birthyear" type="month" format="yyyy年MM月" value-format="yyyy年MM月"
                        placeholder="出生年月" clearable>
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="联系电话" prop="tel" :label-width="labelWidth">
...
export default {
    ...
    data() {
        ...
        var checkTel = (formRules, value, callback) => {
            ...
            // else callback(new Error('请输入合法的手机号码'))
        };
        return {
            // 顶部表单数据
            formData: {
            ...
            formRules: {
                ...
                tel: [
                    ...
                    { pattern: /^1[34578]\d{9}$/, message: '请输入正确的联系电话', trigger: 'blur' },
                    //pattern是正则表达式的方式，validator是定义函数的方式
                    // { validator: checkTel, trigger: 'blur' }
                ],
                marks: [
                    { required: true, message: '请输入可用积分', trigger: 'blur' },
                    { type: 'number', min: 3, message: '可用积分至少100', trigger: 'blur' }//min是至少的长度
                ],
                money: [
    ...
    methods: {
        ...
        getId() {
            let id = 3
            this.$axios.get('/member/getId/' + id).then(res => {
                console.log(res)
                if (res.status == '200') {
        ...
        async deleteRow(row) {
            ...
            // , { data: { id: row.num } }
            this.$axios.delete('/member/delete', {
                params: { id: row.id }
            }).then(res => {
                if (res.status == '200') {
        ...
        edit() {
            this.$refs['editForm'].validate(valid => {
                if (!valid) return false
                this.$axios.post('member/update/' + this.editForm.id, {
        ...
        //查所有
        getList() {
            this.$axios.get('member/getAll').then(res => {
                if (res.status == 200) {
